...
export async function POST(request: Request) {
  ...
  const fumbleTrades = enhancedTrades.map((trade) => {
    ...
    const missedProfitPotential = (() => {
      const potentialValue = athAfterSell.price * trade.total_sold_token_amount;
      const actualValue =
        trade.avg_sell_price_per_token_in_usd * trade.total_sold_token_amount;

      return potentialValue - actualValue;
    })();
  ...
  fumbleTrades.sort(
    (a, b) => b.missed_profit_potential - a.missed_profit_potential,
  );

  const slicedFumbleTrades = fumbleTrades.slice(0, 5);

  return Response.json({
    total_missed_profit_potential: slicedFumbleTrades.reduce(
      (acc, trade) => acc + Math.max(0, trade.missed_profit_potential),
      0,
    ),
    fumble_trades: slicedFumbleTrades as FumbleTradeItem[],
  });
}
