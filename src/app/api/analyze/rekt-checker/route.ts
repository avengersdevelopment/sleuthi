...
import { RektCheckerRequestSchema } from "@/data/schemas/dto";
...
export async function POST(request: Request) {
  ...
  const result = RektCheckerRequestSchema.safeParse(body);
  ...
  return Response.json({
    total_loss_in_usd: rektTrades.reduce(
      (acc, trade) => acc + trade.total_pnl_in_usd,
      0,
    ),
    rekt_trades: rektTrades as RektTradeItem[],
  });
}
